...
class ElasticNetClassifier(CVObject):
    ...
    def determine_alpha_ratio(self, X, Y, fit_intercept=False, jobs=8):
        print 'determining alpha and l1_ratio with crossvalidation...'
        Xnorm = simple_normalize(X)
        clf = ElasticNetCV(l1_ratio=[.1,.3,.5,.7,.8,.9,.95,.99], cv=5,
            fit_intercept=fit_intercept, n_jobs=jobs, verbose=True)
        clf.fit(Xnorm, Y)
        alpha = clf.alpha_ 
    ...
    def fit_elasticnet(self, X, Y, alpha=1.0, l1_ratio=0.5, fit_intercept=False):
        print 'fitting Elastic Net...'
        Xnorm = simple_normalize(X)
        clf = ElasticNet(alpha=alpha, l1_ratio=l1_ratio, fit_intercept=fit_intercept)
        clf.fit(Xnorm, Y)
        return clf
    ...
    def train_svm(self, X, Y, alpha=1.0, l1_ratio=0.5):
        
        print 'Training next group...'
        clf = self.fit_elasticnet(X, Y, alpha=alpha, l1_ratio=l1_ratio)
        return clf
